...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  useEffect(() => {
    const currentIndex = items.findIndex(item => pathname === item.href);
    ...
    <>
      ...
      <style>
        {`
          :root {
            ...
            /* GooeyNav Color Palette - Japanese Matte Colors */
            --gooey-color-1: var(--color-deep-red);
            --gooey-color-2: var(--color-soft-gold);
            --gooey-color-3: var(--color-warm-beige);
            --gooey-color-4: var(--color-primary);
          }
          .effect {
            position: absolute;
            opacity: 1;
          ...
          .dark .gooey-nav-item.active {
            color: #3B3B3B !important;
          }
          .dark .gooey-nav-item.active a {
            color: #3B3B3B !important;
          }
      ...
      <div className="relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ref={navRef}
            className="flex gap-8 list-none p-0 px-20 m-0 relative z-[3]"
            style={{
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] ${activeIndex === index ? 'active' : ''
                  }`}
              >
                <a
                  href={item.href}
