...
async function main() {
    try {
        ...
        const res = await db.query("SELECT last_processed_block FROM sync_state WHERE LOWER(contract_address) = $1", [contractAddress.toLowerCase()]);

        let lastSynced = 0;
        if (res.rowCount > 0) {
            lastSynced = res.rows[0].last_processed_block;
        }

        console.log(`Syncing PledgeMade events from ${lastSynced + 1} to ${currentBlock}`);
        ...
        await db.query("UPDATE sync_state SET last_processed_block = $1 WHERE LOWER(contract_address) = $2", [currentBlock, contractAddress.toLowerCase()]);
