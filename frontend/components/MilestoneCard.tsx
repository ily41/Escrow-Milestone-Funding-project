...
export default function MilestoneCard({ milestone, projectId, onUpdate }: MilestoneCardProps) {
  ...
  const handleOpenVoting = async () => {
    try {
      await openVoting({ milestoneId: milestone.milestone_id || (milestone as any).id }).unwrap()
      onUpdate()
      alert('Voting opened!')
    } catch (error: any) {
      const errorMessage = error?.data?.error || error?.data?.message || error?.error || 'Failed to open voting'
      alert(errorMessage)
    }
  }
  ...
  const handleVote = async (decision: 'approve' | 'reject') => {
    try {
      await vote({ milestone_id: milestone.milestone_id || (milestone as any).id, decision }).unwrap()
      setHasVoted(true)
      onUpdate()
      alert(`Vote submitted: ${decision}`)
    } catch (error: any) {
      const errorMessage = error?.data?.error || error?.data?.message || error?.error || 'Failed to submit vote'
      alert(errorMessage)
    }
  }
  ...
  const handleRelease = async () => {
    ...
    try {
      await releaseFunds({ milestoneId: milestone.milestone_id || (milestone as any).id }).unwrap()
      onUpdate()
      alert('Funds released successfully!')
    } catch (error: any) {
      const errorMessage = error?.data?.error || error?.data?.message || error?.error || 'Failed to release funds'
      alert(errorMessage)
    }
  }
  ...
  const loading = isOpeningVoting || isVoting || isReleasing
    ...
    <>
      ...
      <div className="rounded-lg p-4" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}>
        <div className="flex justify-between items-start mb-2">
          <div>
            <h3 className="text-lg font-semibold" style={{ color: 'var(--text)' }}>
              {milestone.order_index ? `Milestone ${milestone.order_index}: ` : ''}{milestone.title}
            </h3>
        ...
        <div className="flex justify-between items-center text-sm mb-3" style={{ color: 'var(--text)', opacity: 0.7 }}>
          <span>Target: {milestone.target_amount || milestone.funding_amount}</span>
          {milestone.due_date && (
