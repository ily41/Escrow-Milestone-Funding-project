...
import { getMyProjects, createProject, activateProject, deactivateProject } from '@/lib/api'
...
export default function CreatorDashboard() {
  ...
  const handleActivate = async (projectId: number) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Failed to activate project')
    }
  }
  ...
  const handleDeactivate = async (projectId: number) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Failed to deactivate project')
    }
  }
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex gap-4 text-sm" style={{ color: 'var(--text)', opacity: 0.7 }}>
                      ...
                      <span>Goal: {project.currency} {parseFloat(project.goal_amount).toLocaleString()}</span>
                      <span>Pledged: {project.currency} {parseFloat(project.total_pledged || '0').toLocaleString()}</span>
                    </div>
                  </div>
